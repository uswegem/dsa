...
scripts/generate_branch_manager_template.py  regenerates the Branch Manager upload template
frontend/static/templates/  the upload template itself - see "Incremental uploads & period selection"
...
**Upload template**: `frontend/static/templates/branch_manager_upload_template.xlsx`
is a blank, correctly-headed starting point to hand to a Branch Manager -
downloadable straight from the Uploads page (next to "Upload Branch
Manager File"). Required columns are shaded and every header carries an
Excel cell comment explaining it. Generated by
`scripts/generate_branch_manager_template.py`, which re-parses its own
output through `parse_branch_manager_file` before writing - re-run it
whenever `ingest_branch_manager.EXPECTED_COLUMNS` changes
(`tests/test_branch_manager_template.py` catches drift between the two in
CI either way).
